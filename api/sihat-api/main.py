...
def preprocess_files(files: list[FileStorage]):
    ...
    # Add channels until the desired number is reached
    while stacked_images.shape[-1] < 4:
        extra_channel = np.zeros(stacked_images.shape[:-1] + (1,))
        stacked_images = np.concatenate(
            (stacked_images, extra_channel), axis=-1)
